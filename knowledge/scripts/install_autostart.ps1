...
# 用法：powershell -ExecutionPolicy Bypass -File scripts\install_autostart.ps1
...
$command = "powershell -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$startScript`""
schtasks /Create /TN $taskName /TR $command /SC ONLOGON /RL LIMITED /F | Out-Null
if ($LASTEXITCODE -ne 0) {
    Write-Error "计划任务创建失败（退出码 $LASTEXITCODE）"
    exit 1
}
Write-Host "已注册开机自启：$taskName（当前用户登录时自动启动）"
