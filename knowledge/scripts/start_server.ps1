...
if (-not (Test-Path ".venv\Scripts\python.exe")) {
    Write-Error "未找到虚拟环境 .venv。请先按 knowledge/README.md「快速开始」完成安装。"
    exit 1
}
...
Write-Host "启动 EasyShare 知识服务：http://$(if (Test-Path .env) { (Select-String -Path .env -Pattern '^HOST=(.+)$' | ForEach-Object { $_.Matches[0].Groups[1].Value }) } else { '0.0.0.0' }):8000"
Write-Host "同事访问地址：http://<本机IP>:8000/lab    停止：Ctrl+C"
