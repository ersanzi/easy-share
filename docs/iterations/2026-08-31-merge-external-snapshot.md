...
> **勘误（2026-08-31 批次收尾迭代）**：本节写就时未识别出批次内的 `spacemount.go` + `internal/spacedav` 就是 P4「按用户命名空间的资源管理器挂载」的落地实现（本表"支撑模块"一行实际承载了它）。P4 与浮窗空间切换器均已随 b1de4dc 合入并接线登录/登出；"此电脑"云端盘入口已恢复（个人 19082 / 共享 19083，桌面端进程）。下述 KI-5 已在批次收尾迭代删除，整理算法仍待实施。
