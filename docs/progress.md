...
1. **用户侧动作（非开发）**：照 [`company-rollout-guide.md`](company-rollout-guide.md) 在公司部署，建同事账号，真实文档入监听目录，两周看使用率——种子发芽与否的第一手答案
2. **桌面端集成知识问答**（发芽路线下一迭代）：知识入口长进桌面端（登录态/问答面板，打通 Wails 桌面端 ↔ 知识服务）
3. **WPS 最小闭环**：选中段落查知识引用的最小插件（现场演示的"哇"时刻）
4. **2b 文件归属 / 2c 权限感知检索**：owner 落 manifest → 用户可见 doc_ids 注入 /query（钩子已就位）
5. **质量体检报告获客入口**：上传 10 份文档出体检报告（种子发芽后作为接触制造业客户的钩子）
6. **"问过的不再问第二遍"**（候选 idea，定位文档 §三）：问答自动沉淀为知识资产，待真实使用验证形态
7. **1.8 批量压测**（后置）：与其他测试统一做
8. **知识平台里程碑 2 全量**（暂缓）：Java 控制面按需拆分（薄切片路径替代，见定位文档）
