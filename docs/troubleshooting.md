...
9. **`wails dev` 下插件页白屏/插件中心正常但插件全打不开（生产版正常）**：`wails dev` 的前端由 Vite 提供，Vite 对未知路径回 200（SPA fallback），`/plugins/...` 永远轮不到 AssetServer 的 fallback `Handler`，插件 iframe 加载到的是前端应用本身。修复：`/plugins/`、`/clipboard-files/` 走 AssetServer `Middleware` 前置接管（`main.go` + `appplugin.go pluginAssetMiddleware`），dev 与生产行为统一。新增宿主静态前缀时记得同样加进 Middleware。
10. **插件页首开历史/待办为空，但触发一次复制事件后数据又出来了**：iframe 桥接竞态——`PluginHost.vue` 原来在 iframe `@load` 才登记 contentWindow，而 `@load` 在插件脚本执行完之后才触发，插件 app.js 初始化即发起的能力调用被 usePlugins 的来源校验静默丢弃（Promise 永不落地）。修复：登记提前到 iframe 元素插入时（contentWindow 是稳定 WindowProxy，导航后身份不变）。SDK（eshare.js）无握手协议，勿在插件里依赖「首帧调用必达」以外的时序假设前先确认宿主登记时机。
