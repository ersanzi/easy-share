...
export interface TransferTask { id:string; fileName:string; localPath?:string; direction:'send'|'receive'; peer:string; batchId?:string; totalBytes:number; transferredBytes:number; speed:number; status:string; error?:string; createdAt?:string; updatedAt?:string }
