...
2. **WPS 最小闭环**（发芽路线下一步）：选中段落查知识引用的最小插件（现场演示的"哇"时刻；Core 知识网关 API 已就位可直接复用）
3. **2b 文件归属 / 2c 权限感知检索**：owner 落 manifest → 用户可见 doc_ids 注入 /query（钩子已就位）
4. **质量体检报告获客入口**：上传 10 份文档出体检报告（种子发芽后作为接触制造业客户的钩子）
5. **"问过的不再问第二遍"**（候选 idea，定位文档 §三）：问答自动沉淀为知识资产，待真实使用验证形态
6. **1.8 批量压测**（后置）：与其他测试统一做
7. **知识平台里程碑 2 全量**（暂缓）：Java 控制面按需拆分（薄切片路径替代，见定位文档）
